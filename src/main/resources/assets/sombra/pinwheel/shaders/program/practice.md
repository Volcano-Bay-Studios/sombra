...
void main() {
    vec4 source = texture(DiffuseSampler0,texCoord);
    float depth = distance(vec3(0,0,0),screenToLocalSpace(texCoord,texture(DiffuseDepthSampler,texCoord).r).rgb/200);
    vec4 position = screenToWorldSpace(texCoord,texture(DiffuseDepthSampler,vec2(texCoord)).r);
    vec4 color = (vec4((noise(position.x*20)+noise(position.y*20)+noise(position.z*20))/3));
    fragColor = mix(color,vec4(0),clamp(depth,0,1));
}```